...
- **🔌 极简零配置哲学（双模式即插即用）**：
  - 本插件秉承**“极简、透明、即插即用”**的设计原则：100% 遵循标准 Anthropic 原生协议，无需在本地维护繁琐复杂的协议映射表或设置页面；
...
#### 2. Claude 桌面端双重内部校验揭秘（第三方品牌词过滤 + 工具链激活）

根据对客户端接口行为与兼容性的实测验证，客户端内部对 Model ID 存在两套关键制约机制：

1. **规则一：第三方品牌词过滤规则**
   客户端内置了如下正则表达式：
   *(已根据客户端接口规范进行合规化命名映射)*
   - 若模型 ID 命中上述关键字（如 `deepseek-v4`），界面会直接报红：*`Doesn’t look like an Anthropic model`* 并拒绝保存。

2. **规则二：能力梯队与高级工具链激活（关键暗坑）**
   - 即使符合品牌词过滤（例如随意填一个 `custom-model` 或 `test-bot`），客户端内部会根据名称判断模型所属的“能力梯队”（Model Tier）；
   - **若不匹配官方标准的 `claude-sonnet-...` 或 `claude-opus-...` 格式**，客户端会认为该模型“不具备高级编程与工具执行能力”，**在底层静默剔除掉大量核心 Agent 工具（如多文件批量编辑、代码树检索、Subagent 派发等 87+ 工具库大面积丢失，直接退化为普通简易对话）**！

> 💡 **最佳实践**：必须采用我们推荐的标准白名单别名（如 `claude-sonnet-5`、`claude-sonnet-4-8` 等），既完美顺利通过名称合规校验，又能**满血激活 87+ 顶级 Agent 工具链与全自动编程能力**！
