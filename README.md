...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Version-0.1.53-brightgreen.svg" alt="Version">
...
> **让 Claude 桌面版（Code 模式 & Cowork 协作模式）无缝接入 DeepSeek / OpenCode / 各类第三方大模型，稳定保持 99%+ 前缀缓存命中率，无损保留完整思维链（Thinking），首字毫秒级响应，API 费用降低 90%！**
...
- **🔌 极简零配置哲学（双模式即插即用）**：
  - 本插件秉承**“极简、透明、即插即用”**的设计原则：100% 遵循标准 Anthropic 原生协议，无需在本地维护繁琐复杂的设置页面，开箱即用；
  - **双模式全覆盖**：完美支持 Claude 桌面端的 **Code 编程模式** 与 **Cowork (Code Work) 协作模式**（支持子代理 Subagents 并行请求独立追踪与精准统计）。
- **⚡ 字节级前缀稳定化（锁定上下文基准，99%+ 命中率）**：
  ...
  - **`date_pin`（首日日期锁定）**：锁定会话第 1 次创建的日期，彻底消除跨天对话导致的全球全量缓存击穿（自带 `date_pin_regex_miss` 防御告警）；
...
- **🛣️ 生态路线图：专为 Claude Code 定制的 OpenAI 协议转译引擎**：
  - **背景**：目前市面上绝大多数优秀的大模型（如各类国产商业模型、开源模型）默认仅支持 **OpenAI Chat** 协议标准，而 Claude 桌面端（Code 模式）强制要求标准的 **Anthropic Messages** 协议，两者在流式时序、思维链格式与工具签名上存在巨大差异；
  - **自研转译方案**：为了让更多优质大模型能直接接入 Claude 桌面端，我们专门针对 Claude Code 的特性自研了一套**高可用异构协议转译引擎**（深度支持思维链无损还原、流式 Token 对齐与工具交互），目前在私有服务端稳定运行；
  - **后续规划**：为了保持本地插件的极致纯粹与零配置体验，当前插件仅作为标准客户端网关。后续我们将视社区反馈与实际需求，将这套专门为 Claude Code 定制适配的转译引擎以【独立开源网关脚本】或【可选内置插件模块】的形式开放给社区，敬请关注！
...
#### 2. Claude 桌面端模型命名与工具链激活机制说明

根据对客户端接口行为与兼容性的实测验证，Claude 桌面端对 Model ID 的命名有以下两套关键规则：

1. **规则一：第三方品牌词过滤规则**
   - 客户端会对模型名称进行过滤校验，若 Model ID 包含常见的第三方厂商或竞品词汇（如 `deepseek`、`openai`、`mimo`、`qwen` 等），3P 设置界面会提示名称不合规并阻止保存。

2. **规则二：能力梯队与高级工具链激活（关键暗坑）**
   - 客户端内部会根据 Model ID 自动判定模型能力梯队；
   - **若填写的模型名未采用标准的 `claude-sonnet-...` 或 `claude-opus-...` 命名格式**，客户端会将其判定为普通轻量模型，**在底层静默关闭大量核心 Agent 编程工具（导致 87+ 工具库缺失，退化为普通简易对话）**！

> 💡 **最佳实践**：采用我们推荐的标准白名单别名（如 `claude-sonnet-5`、`claude-sonnet-4-8` 等），既能顺利通过名称合规校验，又能**满血激活 87+ 顶级 Agent 工具链与全自动编程能力**！

#### 3. 官方白名单推荐模型表（全量激活 87+ 顶级工具链）

在 **Model list** 区域点击 **+ Add** 添加以下推荐模型。每一个模型项展开后，涉及到三个核心字段的清晰分工：

1. **Model ID (内部模型标识)**：**必须填入官方白名单格式的模型名称**（如 `claude-sonnet-5`、`claude-sonnet-4-8` 等），用于满足客户端严格的命名校验并全量解锁 87+ 顶级 Agent 工具链；
2. **Display name (界面展示名称)**：**建议填入映射的真实上游模型名称**（如 `DeepSeek V4 Flash`、`Xiaomi MiMO 2.5`、`Ox Alpha Free` 等），也可以随意填入您喜欢的任何个性化名称（该字段仅作为客户端 UI 下拉菜单中的视觉展示，不影响底层路由）；
3. **Offer 1M-context variant (100万长上下文开关)**：**建议全部勾选/开启该开关**（开启后在模型选择器中提供 1M 超长上下文选项，充分释放长文本推理潜力）。

| Model ID (`name`)<br>*(必须填官方白名单格式)* | Display name (`labelOverride`)<br>*(建议填真实模型名或自定义)* | Offer 1M-context<br>*(100万上下文)* | 映射转发的真实上游模型 | 场景与特性说明 |
|---|---|:---:|---|---|
| **`claude-sonnet-5`** | `DeepSeek V4 Flash` | ✅ 开启 | `deepseek-v4-flash` | ⚡ 日常极速主力，全量激活 87+ 工具链（强烈推荐） |
| **`claude-opus-5`** | `DeepSeek V4 Pro` | ✅ 开启 | `deepseek-v4-pro` | 🧠 复杂工程架构与极深推理，全量激活 87+ 工具 |
| **`claude-sonnet-4-8`** | `Ox Alpha Free` | ✅ 开启 | `x-preview-f-free` | 🎁 100万长上下文深度推理（完全免费公测中） |
| **`claude-sonnet-4-6`** | `Xiaomi MiMO 2.5` | ✅ 开启 | `mimo-v2.5` | 📱 小米 MiMO 2.5 全能版（多模态 Agent 工具链，白菜价） |
| **`claude-opus-4-6`** | `Xiaomi MiMO 2.5 Pro` | ✅ 开启 | `mimo-v2.5-pro` | 📱 小米 MiMO 2.5 Pro 旗舰版 |

> 💡 **一次配齐，终身免配**：在初次配置时将上述 5 个模型一次性全部添加完毕，后续在聊天界面随时下拉自由切换，日常无需重复配置！
> 想要映射更多自定义模型？在对话中输入 `/cc-config-add <别名> <目标模型>` 即可随时热更新映射表，无需改动源码。
...
### Q4: 电脑重启后，网关没有运行/提示连接失败怎么办？（极简自唤醒）
- **现象**：电脑重启后，本地后台网关进程（8789 端口）不会随系统开机自启，打开 Claude 桌面端时可能会短暂看到红字提示连接不可用。
- **全自动自唤醒（推荐，无需开终端）**：
  - **直接在任意对话框中随意发送一条消息（甚至输入一个字或按回车）**；
  - 插件内置的 `UserPromptSubmit` 钩子会**在 100ms 内瞬间感知并自动拉起本地网关与守护进程**，随后请求自动恢复正常！
- **手动命令行管理（备用）**：
  ```bash
  # 随时随地可通过脚本手动管理本地网关
  python3 scripts/manager.py start    # 启动网关
  python3 scripts/manager.py status   # 查看运行状态与缓存命中率统计
  python3 scripts/manager.py restart  # 重启网关
  ```
